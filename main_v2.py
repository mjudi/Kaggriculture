...
def needs_water(tile):
    """v2: every-other-day watering for ONGOING crops (strawberry, tomato).
    Verified against the engine (_daily_refresh_plants): a crop turns to WEED
    only at consecutive_unwatered >= 2 (survives one missed day), and on a
    surviving-but-unwatered day it STILL accrues its base yield -- watering
    only adds the fertilizer bonus. So watering an ongoing crop only when it's
    already dry (consecutive_unwatered >= 1) keeps it alive on HALF the
    watering actions with ~no yield loss, freeing labor to plant/tend more
    tiles. One-time crops (wheat, carrot, melon) still water every day (short
    maturity window, no production interval to align a skipped day to)."""
    if not (isinstance(tile, dict) and tile.get("kind") == "PLANT"):
        return False
    if tile.get("watered_today"):
        return False
    crop = CROPS.get(tile.get("crop"))
    if crop and crop.get("ongoing", False):
...
def build_market_orders(farm, private, day, hour, prices, has_animals, animal_pick):
    ...
    # v2 FIX: the old batch used `top_crop = eligible[0]`, which is ALWAYS
    # WHEAT (it's first in planting_priority), so STRAWBERRY seed was never
    # batch-bought and its stock sat at 0-1 the whole game -- confirmed
    # directly in a best567 self-play trace: strawberry capped at ~13 tiles
    # while 34-45 tiles sat EMPTY late-game (esp. after the 3rd quadrant was
    # bought and never filled). The winners on the real ladder fill their
    # board (<10 empty tiles); best567 wastes ~45% of its land. Fix: buy
    # seeds by explicit priority to actually FILL open land -- MELON up to its
    # target (early cash), then STRAWBERRY to soak up the rest (the crop meant
    # to dominate), then WHEAT as feed/backfill. Melon and strawberry can no
    # longer be starved by wheat winning the single "top_crop" slot.
    if hour == 0 and empty_tiles > 0:
        want = {}
        if "MELON" in eligible:
            want["MELON"] = max(0, MELON_TARGET - field_counts_now.get("MELON", 0))
        if "STRAWBERRY" in eligible:
            want["STRAWBERRY"] = empty_tiles  # strawberry fills the open land
        if "WHEAT" in eligible:
            want["WHEAT"] = min(empty_tiles, 16)  # feed crop + late-game backfill, capped
        spend = money - RESERVE
        for crop in ["MELON", "STRAWBERRY", "WHEAT"]:
            if crop not in want or len(orders) >= 9:
                continue
            cost = CROPS[crop]["seed_cost"]
            need = max(0, want[crop] - seeds.get(crop, 0))
            if need <= 0 or spend < cost:
                continue
            buy_qty = min(need, int(spend // cost), 10)
            if buy_qty > 0:
    ...
    if len(unlocked) < 3:
        ...
        # v2: utilization gate raised 0.7 -> 0.82. With the fixed seed
        # supply, v2's stronger strawberry economy hits the cash gate sooner
        # and was buying the next quadrant while the current one was only ~70%
        # full -- confirmed on a close loss (seed 3): v2 expanded on day 16
        # vs best567's day 20 and spiked to 35 empty tiles it couldn't fill,
        # costing the game. Only expand once the current land is genuinely
        # near-full so new land actually gets planted rather than sitting idle
        # (idle land is unearned money -- the whole point of the fill fix).
        if day >= 4 and utilization > 0.82 and money - RESERVE >= next_cost:
