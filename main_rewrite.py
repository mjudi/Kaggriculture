...
ANIMAL_PLAN = [("COW", 8), ("SHEEP", 6), ("GOOSE", 1)]
...
# --------------------------------------------------------------- zones ----
# Layer A (routing rewrite): the engine lets a unit either MOVE one tile OR
# act per turn, never both, so every job costs distance+1. The old routing
# assigned each unit the globally-nearest open job, which made units thrash
# across the whole board (finish in NW, get sent to SW, walk 8 tiles for one
# action). With 15 animals across 3 quadrants that wasted most of the
# workforce on travel and strawberry stalled near 9 tiles. Zone ownership
# keeps each unit in a home region so its movement is short hops that
# actually reach the crops/animals it's responsible for -- the same thing
# keiz's real agent does (same 12 hands, same ~43% move rate as ours, yet 50
# strawberry: the difference is WHERE the movement goes, not how much).

def quadrant_of(x, y, board_size):
    """Which 5x5 quadrant a tile is in: 'NW'/'NE'/'SW'/'SE'. Matches the
    engine's own _quadrant_of exactly."""
    half = board_size // 2
    return ("N" if y < half else "S") + ("W" if x < half else "E")


def assign_zones(units, tiles, board_size):
    """Give each unit a home zone (a set of quadrant labels it services).

    Stateless and deterministic: a unit's zone is a pure function of its
    index, the set of unlocked quadrants, and the current per-quadrant
    workload -- so the same unit lands in the same zone turn after turn
    (given stable geometry), without any stored state, and units don't
    oscillate between quadrants.

    Units are distributed across the unlocked quadrants in proportion to
    each quadrant's workload (a quadrant with more pending jobs gets more
    units), so an animal-dense quadrant gets enough feeders and a
    crop-heavy one gets enough waterers. Every unit still gets at least a
    fallback so no unit is left with no zone.

    Returns {unit_name: set_of_quadrant_labels}. The unit services jobs in
    those quadrants first; the caller falls back to the whole board only
    when the unit's zone has no open job this turn.
    """
    unlocked = sorted({quadrant_of(x, y, board_size) for x, y, _ in tiles})
    if not unlocked:
        return {name: set() for name, _, _ in units}

    # Workload per quadrant = count of tiles that represent live work
    # (anything that isn't empty-and-locked); empty plantable tiles count
    # too, since planting them is work. A quadrant with only bare pastures
    # sitting full still counts them lightly. Simple, robust proxy: number
    # of non-None, non-LOCKED tiles + number of empty (plantable) tiles.
    load = {q: 0 for q in unlocked}
    for x, y, t in tiles:
        q = quadrant_of(x, y, board_size)
        if q in load:
            load[q] += 1  # every unlocked tile is potential work (crop, animal, or plantable)

    n_units = len(units)
    # Allocate unit slots to quadrants proportional to load, min 1 each.
    total_load = sum(load.values()) or 1
    alloc = {q: max(1, round(n_units * load[q] / total_load)) for q in unlocked}
    # Adjust so the allocations sum to exactly n_units.
    while sum(alloc.values()) > n_units:
        # take one from the currently most-allocated quadrant (but never below 1)
        q = max((q for q in unlocked if alloc[q] > 1), key=lambda q: alloc[q], default=None)
        if q is None:
            break
        alloc[q] -= 1
    while sum(alloc.values()) < n_units:
        # give one to the highest-load quadrant
        q = max(unlocked, key=lambda q: load[q] / alloc[q])
        alloc[q] += 1

    # Build a flat list of quadrant labels, one entry per unit slot, then
    # hand them to units in index order -- deterministic.
    slots = []
    for q in unlocked:
        slots.extend([q] * alloc[q])
    zones = {}
    for i, (name, _, _) in enumerate(units):
        home = slots[i] if i < len(slots) else unlocked[i % len(unlocked)]
        zones[name] = {home}
    return zones
...
def agent(obs):
    ...
    # Layer A: give each unit a home zone (set of quadrant labels) so its
    # fieldwork stays local -- short hops instead of board-wide hauls. The
    # per-unit job pool below is filtered to the unit's zone first, with a
    # board-wide fallback only when the zone has no open job this turn.
    unit_zones = assign_zones(units, tiles, board_size)
    ...
    for name, pos, inv in units:
        ...
        # Layer A: zone-filter only the DISCRETIONARY, spatially-local work
        # (water, care, harvest, fertilizer, weed, plant) to this unit's home
        # quadrant, so units stop crossing the board for routine fieldwork.
        # But SURVIVAL-CRITICAL jobs stay GLOBAL: a starving animal
        # (needs_feed) or a decaying crop about to be lost (crop_maxed,
        # crop_urgent) anywhere on the board must pull the nearest free unit
        # regardless of zone -- an early Layer-A draft that zoned feed too
        # let herds in an unstaffed quadrant starve to death (minHerd crashed
        # to 2 on some seeds). So feed/decay jobs are computed board-wide and
        # always outrank the zoned local work.
        home = unit_zones.get(name, set())
        in_zone = (lambda x, y: quadrant_of(x, y, board_size) in home) if home else (lambda x, y: True)

        # Global survival tiers (never zone-restricted):
        ...
        def local_tiers(pred_filtered):
            """Zoned discretionary work, with a board-wide fallback if this
            unit's zone has none of that tier open."""
            zoned = [(x, y, t) for x, y, t in tiles
                     if pred_filtered(x, y, t) and (x, y) not in claimed and in_zone(x, y)]
            if zoned:
                return zoned
            return [(x, y, t) for x, y, t in tiles
                    if pred_filtered(x, y, t) and (x, y) not in claimed]

        animalmax = local_tiers(lambda x, y, t: animal_maxed(t))
        water = local_tiers(lambda x, y, t: needs_water(t))
        care = local_tiers(lambda x, y, t: needs_care(t))
        harvest = local_tiers(lambda x, y, t: ready_to_harvest(t, day))
        fert = local_tiers(lambda x, y, t: has_fertilizer_to_collect(t))
        weed = local_tiers(lambda x, y, t: is_weed(t))
        plant = local_tiers(lambda x, y, t: is_plantable(t))
        if underplanted:
